...
let sortedByName2 = deepCopyStudents.sort((a,b)=>a.name.localeCompare(b.name))
let sortedByName =  deepCopyStudents.sort((a,b)=> a.name - b.name)
...
let max = newDeepCopyStudents.reduce((acc,curr) => {
    return acc.scores > curr.scores ? acc:curr
})
...
let scoresSum = newDeepCopyStudents.reduce((acc,value)=>{
    return acc + value.scores
},0)
